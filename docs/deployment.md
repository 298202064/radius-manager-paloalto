...
- 操作系统: Linux（推荐 Ubuntu 22.04+）、macOS 或 Windows（Docker Desktop）
...
将输出的密钥填入 `.env` 的 `ENCRYPTION_KEY` 字段。`SECRET_KEY` 使用任意强随机字符串即可。
...
1. 进入 **在线用户** 页面
2. 实时显示当前 GlobalProtect VPN 在线用户列表
...
| POST | /api/v1/nas-clients/sync | Admin | 手动同步配置到 FreeRADIUS |
...
API 文档（需通过 Nginx 访问）: http://localhost:8081/docs
